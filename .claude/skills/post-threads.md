...
## 前提
- `.env` に Meta トークン（`META_ACCESS_TOKEN`, `META_THREADS_USER_ID`）が設定済み
...
3. frontmatter の `approved` フィールドを確認:
   ...
   - `approved: true` なら次へ。
4. Bash で `python3 scripts/post_threads.py --date ${date} --commit` を実行する。
...
## 失敗時の挙動
- **401 / トークン期限切れ**: 「Threads アクセストークンが期限切れです。`python3 scripts/meta_token_refresh.py --commit` を実行してください」と案内。
